...
// остальное (Приз/Чат/Игроки) и "Ход —" в неё больше не пишем: сам ход теперь
// виден отдельными сообщениями в чате (см. announceTurn ниже). "Распорядитель"
// сюда никогда не пишем — она уже есть в предшествующем замороженном
// лобби-сообщении, дублировать её тут незачем.
function renderGameMessage(battle) {
  ...
  // "Распорядитель битвы" тут намеренно не дублируется — эта строка уже
  // стоит последней в "замороженном" лобби-сообщении прямо перед этим (см.
  // renderLobbyMessage/syncLobby(frozen=true)), а следом сразу идёт отдельное
  // "Пистолет заряжен, начинаем." — повторять её здесь было лишним.
